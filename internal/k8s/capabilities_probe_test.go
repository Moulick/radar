...
// TestProbeResourceAccess_MixedScope verifies the all-or-nothing bug fix.
//
// Before: if even one namespaced resource passed the cluster-wide check
// (e.g. Events broadly readable), the namespace-scoped retry was skipped
// for *every* other resource — leaving the user with only Events visible.
//
// After: each kind is probed independently. Events stays cluster-wide; the
// other kinds correctly fall back to the namespace.
...
// informers — without this, the cache would still be cluster-wide and
// the picker would silently do nothing.
//
// Cluster-scoped kinds (nodes, namespaces, PV, storageclasses) must end
// up disabled in this mode — they can't live in a single namespace.
func TestProbeResourceAccess_ForceNamespaceClusterWideUser(t *testing.T) {
	...
	for _, k := range []string{k8score.Nodes, k8score.Namespaces, k8score.PersistentVolumes, k8score.StorageClasses} {
		if scopeOf(result, k).Enabled {
			t.Errorf("%s should be disabled under forceNamespace (cluster-only kind)", k)
		}
	}
}
