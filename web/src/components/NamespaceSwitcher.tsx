...
/**
 * NamespaceSwitcher narrows or widens the cluster cache to a single namespace.
 * Pairs with ContextSwitcher in the header.
 *
 ...
 *     surfaces only the kubeconfig context's namespace + any saved override.
 *
 * Re-init time is non-trivial (informers re-probe + re-sync) so the trigger
 * shows a spinner during the swap and we suppress repeat clicks.
 */
        ...
        content={
          restrictedHint
            ...
            : isClusterWide
              ...
              : `Cache is scoped to namespace ${scope.active}. Click to switch or reset.`
        }
        delay={300}
        position="bottom"
      >
        <button
          ref={triggerRef}
          onClick={() => !setActive.isPending && setIsOpen(o => !o)}
      ...
      {isOpen &&
        createPortal(
          <div
            ...
            className="bg-theme-surface border border-theme-border rounded-md shadow-lg overflow-hidden"
          >
            {items.length > 6 && (
            ...
            {!scope.authoritative && (
              <div className="px-3 py-2 border-t border-theme-border text-[11px] text-amber-400">
